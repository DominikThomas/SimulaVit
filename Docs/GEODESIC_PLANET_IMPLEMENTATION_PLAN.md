...
### Normal-world startup calibration (2026-08)

Normal Geodesic startup uses conservative global geological source defaults of **CO2 0.02**, **H2 0.006**, **H2S 0.004**, and **Fe2 0.002 inventory units per simulated second**. These values are deliberately gentle baselines: CO2 is non-zero but low, H2 remains at its already-modest rate, H2S is reduced, and Fe2 remains at the established low value. Scene-serialized defaults and code fallbacks use the same values so resetting startup configuration cannot restore the former high scene rates.

Atmosphere authoring is intentionally split into two modes. Normal startup authors total pressure plus composition over **0–5 bar**. Deliberate dense/Venus-like worlds remain available only through the Advanced **Allow Dense Atmosphere** opt-in, which expands total pressure authoring to **600 bar**. Legacy schema-v6 partial pressures continue to migrate explicitly to total pressure and fractions; migrated worlds above 5 bar retain the dense opt-in, while existing schema-v7 values and saved vent settings remain user-authored data rather than being silently overwritten by new defaults.

This pass is a calibration and startup-UX correction only. It does **not** change conservation, geological source, air-sea exchange, atmosphere/climate coupling, or terrestrial/submarine vent budget-split equations or semantics. Manual Unity Play Mode validation remains required: compare a default Geodesic run's temperature trend and ocean CO2/H2/H2S rise against the previous defaults; confirm the normal pressure control is limited to 0–5 bar; enable Advanced dense authoring and confirm an intentional Venus-like pressure can still be entered, applied, saved, and reloaded; and verify the startup log reports the effective vent rates and `Normal (0-5 bar)` or `Advanced dense (0-600 bar)` authoring mode.

Ocean startup CO2/O2/CH4/Fe2 remain dissolved concentrations and are configured independently from atmospheric starting pressure/composition. The normal atmosphere UI owns a total initial pressure (range 0–5 bar, default 0.85 bar) plus CO2/O2/CH4/H2/H2S fractions; N2 is the non-negative remaining fraction, trace fractions are proportionally normalized if their sum exceeds one, and every resulting partial pressure is displayed. The default is approximately 0.80 bar N2 plus 0.05 bar CO2. An explicit Advanced opt-in raises the authoring limit to 600 bar (matching the former six 0–100 bar controls' maximum total) for deliberate dense/Venus-like cases. Saved startup schema v7 persists total pressure and fractions; schema-v6 partial pressures are explicitly migrated by summing them and dividing each trace gas by that total, with dense opt-in automatically retained above 5 bar. Old partial pressures are never silently reinterpreted as fractions.

The normal `atmosphereInventoryPerBar` default is now 1000 inventory units/bar (previously 100) and remains configurable in Advanced settings. It is an explicit simulation scaling convention, not a physically calibrated atmospheric-mass claim. Increasing it slows pressure change from a fixed inventory flux by tenfold without changing submarine/terrestrial vent production budgets or air-sea inventory conservation. All equilibrium-concentration-per-bar coefficients remain 1, and the default exchange half-life remains zero (disabled). No ocean value initializes atmosphere or vice versa.

The normal startup ocean remains intentionally disequilibrated when air-sea exchange is enabled: ocean CO2 concentration 1 versus 0.05 bar atmospheric CO2 and coefficient 1 implies equilibrium surface concentration 0.05. This produces a legitimate early ocean-to-atmosphere degassing pulse if the user enables a positive exchange half-life; the default zero half-life keeps exchange disabled. The exchange equation is not tuned to hide this initial condition, and the startup diagnostics expose both atmosphere settings and the half-life.
...
The resource operator order is now: compact submarine vent injection to ocean -> compact terrestrial vent injection to atmosphere -> finite atmosphere/L0 exchange -> horizontal dissolved transport -> vertical dissolved transport -> staged dissolved-state application -> local abiotic chemistry -> chemistry-owned sediment deposition. Thus newly emitted terrestrial gas may enter ocean L0 through exchange and then transport/react in the same resource tick, while deep vent gas must transport to L0 before outgassing.

Atmosphere lifecycle state and exchange diagnostics are reset on teardown and reconstructed exclusively from new-world startup configuration. Read-only `TotalPressureBar`, `GetPartialPressureBar`, and `GetInventory` APIs are the thermal boundary, including CO2 and CH4 partial pressures.

The first atmosphere/climate coupling is implemented only for the production `ApproximateEcologicalProfiles` path and reads those authoritative values on every thermal tick (never only at world initialization). It is a bounded simulation-scale approximation, not an Earth-calibrated climate model. For total pressure `P`, `inertiaMultiplier = clamp(sqrt(max(P, 1e-6 bar) / referencePressureBar), minMultiplier, maxMultiplier)`, with defaults `referencePressureBar=1`, `minMultiplier=0.25`, and `maxMultiplier=4`. The multiplier scales the existing heating/cooling relaxation timescale, so low pressure responds faster and high pressure slower; total pressure does not change equilibrium temperature. N2, O2, H2, and H2S have no greenhouse term.

For each greenhouse gas, `gasDeltaK = clamp(sensitivityK * ln(1 + partialPressureBar / referenceBar), 0, gasMaxWarmingK)`. CO2 defaults are reference `0.1 bar`, sensitivity `2 K`, maximum `30 K`; CH4 defaults are reference `0.01 bar`, sensitivity `0.75 K`, maximum `8 K`. `greenhouseDeltaK = clamp(co2DeltaK + ch4DeltaK, 0, 30 K)`. These give approximate CO2 warming of 0.81 K at 0.05 bar, 1.39 K at 0.10 bar, 4.80 K at 1 bar, 7.86 K at 5 bar, and 9.23 K at 10 bar. The existing local latitude/day-night/seasonal/insolation target is preserved and becomes `atmosphereAdjustedLocalTargetK = existingLocalEquilibriumTargetK + greenhouseDeltaK`; terrestrial vent influence remains layered on that environmental target. Surface relaxation then uses `response = 1 - exp(-dt / ((heatingOrCoolingTimescale * landOrOceanSurfaceMultiplier) * inertiaMultiplier))`. Ocean layer 0 continues to read the surface field and approximate subsurface profiles continue their existing slow depth-dependent relaxation; no independent atmosphere-ocean temperature system was added.

The approximate abyssal reference now has explicit Kelvin semantics. With default `climateReferenceBaseKelvin=273.15 K`, `globalClimateAnomalyK = BaseTemperatureKelvin + greenhouseDeltaK - climateReferenceBaseKelvin` and `abyssalTargetK = clamp(abyssalBaselineC + 273.15 K + globalClimateAnomalyK * abyssalClimateCoupling, 0, 10000 K)`. The default reference preserves the prior normal 273.15 K configuration without treating Celsius conversion as an anomaly. The deep target is refreshed each thermal callback, while existing depth-dependent response and bottom-focused vent heating remain slow and unchanged.

O(1) thermal diagnostics expose live total pressure, pressure inertia multiplier, separate CO2/CH4 contributions, combined greenhouse delta, and effective climate base target. The Environment -> Atmosphere summary includes the combined and per-gas greenhouse values plus inertia. Chemistry, circulation, weather, water vapour, clouds, escape, and greenhouse forcing from gases other than CO2/CH4 remain unimplemented. Unity play-mode validation remains required.

Atmosphere v1 diagnostics expose a compact global pressure/composition summary for both land and ocean cell selections and as the alternate Atmosphere tab of the Geodesic Environment HUD. Inventory-per-bar scale, configured effective surface-L0 relaxation half-life, completed exchange ticks, and cumulative net transfer to the ocean for all five exchangeable gases remain Detailed Debug only. All displays read the authoritative global atmosphere directly; the existing throttled chemistry telemetry appends the same O(1) state without adding a logger or ocean traversal. The air-sea half-life is explicitly presented as the relaxation timescale of surface-ocean L0 concentration toward atmosphere-controlled equilibrium, not as depletion half-life of the finite reservoir; zero remains exchange-disabled.
...
Raw Geodesic geothermal candidates are now deterministic generation-only data. Strongest-first angular clustering, with cell index as the tie-breaker, creates the single authoritative vent-system dataset; submarine and terrestrial candidates are clustered separately so a coastline cannot merge their source habitats. A system's representative is its strongest candidate, its simulation weight is the sum of member raw strengths, and weights normalize to one within each independently configured source habitat. Compact terrestrial outlets now emit CO2, H2, and H2S into the authoritative global `GeodesicAtmosphereField`; they never emit into ocean or Legacy atmosphere, and Fe2 remains submarine/ocean-only.

The configured H2, H2S, and CO2 rates remain global budgets. Their submarine/terrestrial production split is the ratio of summed raw clustered-system strength in each habitat across both habitats; the independently normalized within-habitat system and compact-outlet weights then distribute that habitat share. Consequently `terrestrialVentFraction` controls which eligible land candidates exist but is not itself treated as a production fraction. With only one populated habitat its gas share is one, and in mixed worlds the submarine ocean source plus terrestrial atmospheric source equals the configured global gas rate within floating-point tolerance. Fe2 retains its prior full configured budget distributed only across authoritative submarine compact outlets.
...
## Geodesic physical environmental scale

Geodesic simulation geometry has an explicit physical interpretation: **1 Unity length unit = 1000 km**. The base radius of 8 Unity units therefore represents 8000 km; one Unity² represents 1,000,000 km² and one Unity³ represents 1,000,000,000 km³. This authority is simulation data (`GeodesicPhysicalScale`) and is never inferred from `Transform.localScale` or another rendering setting.

The generated bathymetry and layered-ocean geometry remain deliberately exaggerated for visual and ecological layer separation; they are not intended to reproduce Earth-like bathymetry. Mesh positions, layer radii, and all geometric Unity-space arrays remain unchanged. Physical layer volume is obtained by scaling the existing spherical-wedge volume, not by replacing it with a planar approximation:

`physicalLayerVolumeKm3 = geometricSphericalWedgeVolumeUnity3 * 1,000,000,000`

Dissolved-resource inventory, transport writeback, vent dilution, chemistry, air-sea exchange, chemistry telemetry, and volume-derived subsurface thermal capacity now consume the authoritative physical volume. Concentrations remain `float`; physical volumes, concentration-times-volume inventories, totals, and reductions use `double`. This shared environmental authority is intended for future volume-dependent systems such as thermal capacity rather than as a biology-only multiplier. Fixed vent inventory/time rates and biology/reaction coefficients are intentionally not rebalanced, so their concentration effect is one-billionth of the former raw-Unity-volume interpretation for the same geometry.

Physical scaling preserves relative node volumes but does not make computational cells resolution-independent. For an equal-area representative habitat with equal layer thickness, subdivision 5 has 10,242 cells and subdivision 6 has 40,962 cells; representative area, volume, equal-concentration inventory, and supported Hydrogenotrophy reference-tick reactions differ by `40962 / 10242 = 3.999414...`. A focused EditMode diagnostic reports these quantities rather than hiding the dependency with biology-specific normalization.

The Geodesic atmosphere retains `atmosphereInventoryPerBar` as a pre-physical-scale authoring capacity for startup configuration compatibility. Runtime converts it into the same `concentration * km3` inventory unit as the ocean: `physicalAtmosphereInventoryPerBar = authoringInventoryPerBar * PhysicalCubicKilometresPerUnityUnitCubed`. Thus the default authoring value 1000 becomes `1e12` physical inventory units/bar, while configured initial partial pressures remain unchanged. Air-sea and terrestrial-vent transfers are physical inventory amounts; fixed vent rates are not rescaled.
