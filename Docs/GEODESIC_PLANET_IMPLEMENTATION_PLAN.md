...
The recurring environment-traversal audit found two local same-domain fusions. First, each thermal tick now calculates a cell's solar/geothermal target and immediately applies that cell's surface response in one all-cell pass. Target calculation has no cross-cell dependency, resource state cannot change during the synchronous loop, and every target is still stored before the ocean callback, so operator order and the approximately two-simulated-second cadence are unchanged. Initialization retains its target-only pass. Second, the existing five-simulated-second resource diagnostic refresh now accumulates all seven global resource summaries and the five O2 layer means in one active-node pass, using initialization-owned scratch arrays and retaining each summary's original active-node accumulation order. These changes remove one complete all-planet-cell traversal per steady-state thermal tick and replace twelve diagnostic traversals (seven active-node resource passes plus five all-cell layer passes) with one active-node pass per resource diagnostic refresh. These are static traversal-count reductions, not claims of runtime speedup without Unity profiling. Ocean relaxation, resource transport/staged application, candidate chemistry, throttled detailed telemetry, and revision-driven render updates remain intentionally separate because their domains, cadences, authority, or dependencies differ.
