...
The current web is **partly coherent for gameplay**: vent/bottom Hydrogenotrophy seeds can lead to photosynthesis, oxygen/OrganicC-gated saprotrophy, methane cycling, and predators. However, the sequence is biologically compressed. Photosynthesis is directly reachable from Hydrogenotrophy as oxygenic photosynthesis, while anoxygenic photosynthesis and iron/sulfur phototrophy are only reaction-scaffold placeholders. O2 inhibition is now tracked for configured anaerobes, so oxygen-crisis pressure primarily reduces metabolism efficiency rather than normally killing anaerobes directly.

## 2. Source files inspected

| File path | Relevant classes/enums/methods | Evidence found |
|---|---|---|
| `Assets/Scripts/Biology/Replicators/ReplicatorData.cs` | `MetabolismType`, `LocomotionType`, `DeathCause`, `Replicator`, `Replicator.Traits`, `GetDefaultPreferredOceanLayerIndex` | Authoritative organism enum values, stored traits/timers, layer fields, default layer preference. |
| `Assets/Scripts/Biology/Replicators/ReplicatorManager.cs` | Inspector settings; `SpawnInitialPopulation`, `SpawnAgentFromPopulation`, `SpawnAgentAtRandomLocation`, `SpawnAgentAtDirection`, `ResolveSpawnLayers`, `AssignTemperatureTraits`, `CanMutateToSaprotrophy`, `CanMutateToMethanotrophy`, `CanMutateToPredation`, `ResolveInheritedLocomotion`, `ResolveEnergyDeathCauseAtIndex` | Reproduction mutation rules, spawn defaults, inheritance behavior, temperature-band mutation, layer placement, starvation attribution. |
| `Assets/Scripts/Biology/Replicators/ReplicatorMetabolismSystem.cs` | `MetabolismTick`, `Process*ReactionBacked`, `TryPhotosynthesisEnergyGain`, `TryPhotosynthAnoxicDarkFallback`, starvation timer updates | Runtime resource consumption/production, energy/carbon storage, death pressure, O2 inhibition scope, photosynthesis light/layer behavior. |
| `Assets/Scripts/Biology/Replicators/ReactionBackedMetabolismScaffolding.cs` | `ReactionId`, `ReactionDefinitionRegistry`, packages for every `MetabolismType` | Reaction-backed resource identities; explicitly marked scaffold/non-authoritative; placeholders for photoferrotrophy/anoxygenic photosynthesis. |
| `Assets/Scripts/Biology/Replicators/ReplicatorPredationSystem.cs` | `RunPredationPass`, `ApplyPredationBite`, layer-binned prey search | Predator feeding is prey-contact backed when scent predation is enabled, with same/adjacent layer search. |
| `Assets/Scripts/Biology/Replicators/ReplicatorDebugTelemetry.cs` | `FormatPopulation`, `FormatDeathCauses`, labels | Debug labels expose intended categories/unlocks and death-cause telemetry. |
| `Assets/Scripts/Planet/Common/Generation/PlanetGenerator.cs` | biology unlock settings/properties | Photosynthesis and Saprotrophy unlock by elapsed seconds. |
| `Assets/Scripts/UI/PlanetCellInspectorPanel.cs` | cell/layer inspector summary | UI reveals resource/layer chemistry useful for manual audit. |
...
| O2 tolerance | `o2ComfortMax`, `o2StressMax` | Replicator fields; Methanotrophy metabolism uses them | **No mutation found** | Constructor defaults both to 1; snapshots persist | O2 inhibition telemetry is handled for configured anaerobes; legacy O2 direct-damage timing is reserved for optional direct oxidative damage. |
| Starvation timers | CO2, H2S, H2, light, OrganicC food, O2, CH4, stored C, O2 inhibition/damage | `Replicator` fields; metabolism system; death attribution | **Runtime state, not genetic** | New agents reset; save/load persists | Timers shape selection by attributing energy-depletion death causes. |
...
| Methanotrophy | CH4 + O2 | CO2; some CH4 assimilated to store | CH4 oxidation | CH4 | Yes | No | Yes, assimilation fraction | Lack CH4, O2, stored capacity flag, O2 inhibition/damage, temp efficiency/death | O2 + methane interface inferred; mutation requires local O2 and global CH4 | `CanMutateToMethanotrophy`, `ProcessMethanotrophyReactionBacked` |
...
* **Oxygen crisis / O2 inhibition: implemented for existing anaerobes.** `DeathCause.O2_Toxicity` exists only for optional extreme direct oxidative damage, while Hydrogenotrophy, Fermentation, Methanogenesis, and SulfurChemosynthesis now use configurable runtime anaerobe O2 inhibition thresholds and efficiency floors.
...
| O2 inhibition now shapes existing anaerobes | Hydrogenotrophy, Fermentation, Methanogenesis, and SulfurChemosynthesis use configurable runtime O2 inhibition; Methanotrophy keeps separate handling | Oxygenation can produce an ecological crisis without changing mutation gates. |
...
| 2 | O2 inhibition for anaerobes | Enables oxygen crisis and post-photosynthesis selection pressure | Apply O2 efficiency inhibition to Hydrogenotrophy, Fermentation, Methanogenesis, maybe SulfurChemosynthesis above configurable thresholds | Evolve O2 tolerance/detox trait with cost and staged aerobic transition |
...
graph TD
    ...
    OxygenicPhoto -->|Recommended selection pressure| O2Crisis[O2 inhibition pressure on anaerobes]
