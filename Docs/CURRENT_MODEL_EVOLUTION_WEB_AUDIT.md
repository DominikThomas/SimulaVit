...
* **Oxygen crisis / O2 toxicity: implemented for existing anaerobes.** `DeathCause.O2_Toxicity` exists, Methanotrophy keeps its existing O2 timer, and Hydrogenotrophy, Fermentation, Methanogenesis, and SulfurChemosynthesis now use configurable runtime anaerobe O2 toxicity thresholds/timers.
...
| O2 toxicity now shapes existing anaerobes | Hydrogenotrophy, Fermentation, Methanogenesis, and SulfurChemosynthesis use configurable runtime O2 toxicity; Methanotrophy keeps separate handling | Oxygenation can produce an ecological crisis without changing mutation gates. |
