...
public class GeodesicCellPicker : MonoBehaviour
{
    ...
    private void CacheSelectedStaticPopupText(int cellIndex)
    {
        ...
        compactHeader.Append("Cell index: ").Append(cellIndex)
            .Append("\nClassification: ").Append(planetGenerator.GetGeodesicCellSurfaceClassification(cellIndex))
        ...
        detailed.Append("Cell\nIndex: ").Append(cellIndex).Append("\nType: ").Append(selectedIsPentagon ? "Pentagon" : "Hexagon")
            ...
            .Append("\n\nLakes\n").Append(planetGenerator.GetGeodesicLakeCellDiagnostics(cellIndex)).Append("\nterrainBelowSeaLevel: ").Append(planetGenerator.IsGeodesicCellBelowSeaLevel(cellIndex)).Append("\noceanConnected: ").Append(ocean).Append("\npotentialLakeBasin: ").Append(planetGenerator.IsGeodesicCellPotentialLakeBasin(cellIndex)).Append("\nCoastal land: ").Append(planetGenerator.IsGeodesicCellCoastalLand(cellIndex)).Append("\n\nOcean\nClass: ").Append(planetGenerator.GetGeodesicCellOceanClassification(cellIndex)).Append("\nCoastline: ").Append(planetGenerator.IsGeodesicCellCoastline(cellIndex))
    ...
    private void RefreshDynamicPopupText(bool force = false)
    {
        ...
        selectedCompactPopup = selectedCompactStaticHeader + compactDynamic + selectedCompactStaticOcean + compactLayers + BuildCompactAtmosphereText();
        if (hasExperiencedTemperature)
    ...
    private string BuildCompactAtmosphereText()
    {
        ...
        text.Append("\n\nAtmosphere — global")
            .Append("\nTotal pressure: ").Append(atmosphereField.TotalPressureBar.ToString("G6")).Append(" bar")
            .Append("\nN2: ").Append(atmosphereField.GetPartialPressureBar(GeodesicAtmosphericGas.N2).ToString("G6"))
            .Append(" | CO2: ").Append(atmosphereField.GetPartialPressureBar(GeodesicAtmosphericGas.CO2).ToString("G6"))
            .Append(" | O2: ").Append(atmosphereField.GetPartialPressureBar(GeodesicAtmosphericGas.O2).ToString("G6"))
            .Append("\nCH4: ").Append(atmosphereField.GetPartialPressureBar(GeodesicAtmosphericGas.CH4).ToString("G6"))
            .Append(" | H2: ").Append(atmosphereField.GetPartialPressureBar(GeodesicAtmosphericGas.H2).ToString("G6"))
            .Append(" | H2S: ").Append(atmosphereField.GetPartialPressureBar(GeodesicAtmosphericGas.H2S).ToString("G6"));
    ...
    private void DrawPopupHeader()
    {
        ...
        string classification = planetGenerator != null ? planetGenerator.GetGeodesicCellSurfaceClassification(selectedCellIndex) : "Land";
