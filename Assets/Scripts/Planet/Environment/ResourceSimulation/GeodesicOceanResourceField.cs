...
/// Values are concentrations per layer volume; inventory is concentration * GeodesicOceanLayerGrid.PhysicalLayerVolumeKm3[node].
...
public sealed class GeodesicOceanResourceField : MonoBehaviour
{
    ...
    [SerializeField, Min(0f), Tooltip("Historical authoring rate in concentration*Unity^3 per simulated second. Runtime converts it once to physical inventory.")] private float ventH2PerTick = 0.006f;
    [SerializeField, Min(0f), Tooltip("Historical authoring rate in concentration*Unity^3 per simulated second. Runtime converts it once to physical inventory.")] private float ventH2SPerTick = 0.004f;
    [SerializeField, Min(0f), Tooltip("Historical authoring rate in concentration*Unity^3 per simulated second. Runtime converts it once to physical inventory.")] private float ventCO2PerTick = 0.02f;
    [SerializeField, Min(0f), Tooltip("Historical authoring rate in concentration*Unity^3 per simulated second. Runtime converts it once to physical inventory.")] private float ventFe2PerTick = 0.002f;
    ...
    [SerializeField] private double physicalVentH2InventoryRatePerSecond;
    [SerializeField] private double physicalVentH2SInventoryRatePerSecond;
    [SerializeField] private double physicalVentCO2InventoryRatePerSecond;
    [SerializeField] private double physicalVentFe2InventoryRatePerSecond;
    ...
    private double[] activeNodeVolumes;
    ...
    private void Awake() { domain = GetComponent<GeodesicOceanLayerDomain>(); planetGenerator = GetComponent<PlanetGenerator>(); simulationClock = FindFirstObjectByType<ReplicatorManager>(); ResolveChemistryComponents(); ResolveAtmosphereComponents(); }
    ...
    public bool InitializeForCurrentDomain()
    {
        using (InitializeMarker.Auto())
        {
            ...
            ResolveChemistryComponents(); ResolveAtmosphereComponents(); sedimentField.Initialize(cellCount); abioticChemistry.ResetCounters();
            concentrationsByResourceThenNode = new float[ResourceCount * nodeCapacity];
            activeNodeIndices = new int[activeNodeCount]; activeNodeVolumes = new double[activeNodeCount];
            ...
            {
                int node = grid.GetNodeIndex(cell, layer); activeNodeIndices[cursor] = node; activeNodeVolumes[cursor] = grid.PhysicalLayerVolumeKm3[node]; volume += grid.PhysicalLayerVolumeKm3[node];
                for (int r = 0; r < ResourceCount; r++) concentrationsByResourceThenNode[r * nodeCapacity + node] = configuredInitialConcentrations[r];
                cursor++;
            }
            activeOceanVolume = volume; approximateRuntimeMemoryBytes = (long)concentrationsByResourceThenNode.Length * sizeof(float) + (long)activeNodeIndices.Length * sizeof(int) + (long)activeNodeVolumes.Length * sizeof(double) + (long)chemistryCandidateNodes.Length * sizeof(int) + ResourceCount * 32L;
            ...
            atmosphereField.InitializeForWorld(); airSeaGasExchange.InitializeForWorld(atmosphereField);
            ...
            Debug.Log($"[GeodesicOceanResource] initialized dissolved-ocean concentrations (not atmosphere; not legacy normalized totals): cells={cellCount}, nodeCapacity={nodeCapacity}, activeNodes={activeNodeCount}, physicalVolumeKm3={activeOceanVolume:G6}, memory={approximateRuntimeMemoryBytes} bytes, CO2={initialCO2Concentration:G6}, O2={initialO2Concentration:G6}, CH4={initialCH4Concentration:G6}, H2=0, H2S=0, Fe2={initialFe2Concentration:G6}, OrganicC=0, sentinel={sentinel}", this);
    ...
    {
        // Single authoring-to-runtime boundary: all downstream habitat budgets and injections
        // use physical concentration*km3/s rates and must not apply this scale again.
        physicalVentH2InventoryRatePerSecond = GeodesicPhysicalScale.PhysicalInventoryRate(ventH2PerTick);
        physicalVentH2SInventoryRatePerSecond = GeodesicPhysicalScale.PhysicalInventoryRate(ventH2SPerTick);
        physicalVentCO2InventoryRatePerSecond = GeodesicPhysicalScale.PhysicalInventoryRate(ventCO2PerTick);
        physicalVentFe2InventoryRatePerSecond = GeodesicPhysicalScale.PhysicalInventoryRate(ventFe2PerTick);
        ...
        for (int i = 0; i < horizontalLinkCount; i++) { int a = grid.HorizontalNodeA[i], b = grid.HorizontalNodeB[i]; float geometry = horizontalGeometry[i]; horizontalConductanceBase[i] = (float)(geometry * Mathf.Min(grid.LayerVolume[a] / horizontalSum[a], grid.LayerVolume[b] / horizontalSum[b]) * GeodesicPhysicalScale.PhysicalCubicKilometresPerUnityUnitCubed); }
        ...
        for (int i = 0; i < verticalLinkCount; i++) { int a = grid.VerticalUpperNode[i], b = grid.VerticalLowerNode[i]; float geometry = verticalGeometry[i]; verticalConductanceBase[i] = (float)(geometry * Mathf.Min(grid.LayerVolume[a] / verticalSum[a], grid.LayerVolume[b] / verticalSum[b]) * GeodesicPhysicalScale.PhysicalCubicKilometresPerUnityUnitCubed); }
        ...
        for (int i = 0; i < diagnosticDurations.Length; i++)
        {
            ...
            Debug.Log($"[GeodesicVentInventoryScale] seconds={duration:G6}, inventoryPhysical(H2/H2S/CO2/Fe2)={physicalVentH2InventoryRatePerSecond * duration:G6}/{physicalVentH2SInventoryRatePerSecond * duration:G6}/{physicalVentCO2InventoryRatePerSecond * duration:G6}/{physicalVentFe2InventoryRatePerSecond * duration:G6}, inventoryUnit=concentration*km3, wholeOceanMeanDelta(H2/H2S/CO2/Fe2)={physicalVentH2InventoryRatePerSecond * duration * inverseVolume:G6}/{physicalVentH2SInventoryRatePerSecond * duration * inverseVolume:G6}/{physicalVentCO2InventoryRatePerSecond * duration * inverseVolume:G6}/{physicalVentFe2InventoryRatePerSecond * duration * inverseVolume:G6}, activeOceanVolumeKm3={activeOceanVolume:G6}", this);
        }
    ...
    private void TickResources(float dt)
    {
        ...
        {
            // Deterministic operator split: sources, L0 air-sea exchange, dissolved transport,
            // staged application, local chemistry, and chemistry-owned sediment deposition.
    ...
    private void InjectVentSources(float tickScale)
    {
        ...
        using (VentMarker.Auto()) for (int i = 0; i < ventOutlets.Length; i++)
        {
            ...
            double scale = submarineGasProductionFraction * outlet.SystemBudgetWeight * outlet.WithinSystemWeight;
            double volume = sourceGrid.PhysicalLayerVolumeKm3[node];
            concentrationsByResourceThenNode[(int)GeodesicOceanResource.H2 * nodeCapacity + node] += (float)GeodesicPhysicalScale.VentConcentrationDelta(physicalVentH2InventoryRatePerSecond * scale, tickScale, volume);
            concentrationsByResourceThenNode[(int)GeodesicOceanResource.H2S * nodeCapacity + node] += (float)GeodesicPhysicalScale.VentConcentrationDelta(physicalVentH2SInventoryRatePerSecond * scale, tickScale, volume);
            concentrationsByResourceThenNode[(int)GeodesicOceanResource.CO2 * nodeCapacity + node] += (float)GeodesicPhysicalScale.VentConcentrationDelta(physicalVentCO2InventoryRatePerSecond * scale, tickScale, volume);
            concentrationsByResourceThenNode[(int)GeodesicOceanResource.Fe2 * nodeCapacity + node] += (float)GeodesicPhysicalScale.VentConcentrationDelta(physicalVentFe2InventoryRatePerSecond * outlet.SystemBudgetWeight * outlet.WithinSystemWeight, tickScale, volume);
        }
        double terrestrialScale = terrestrialGasProductionFraction * terrestrialOutletWeight * tickScale;
        atmosphereField.AddGeologicalSource(GeodesicAtmosphericGas.CO2, physicalVentCO2InventoryRatePerSecond * terrestrialScale);
        atmosphereField.AddGeologicalSource(GeodesicAtmosphericGas.H2, physicalVentH2InventoryRatePerSecond * terrestrialScale);
        atmosphereField.AddGeologicalSource(GeodesicAtmosphericGas.H2S, physicalVentH2SInventoryRatePerSecond * terrestrialScale);
    }
    ...
    internal double GetRawConcentration(GeodesicOceanResource resource, int node) => concentrationsByResourceThenNode[(int)resource * nodeCapacity + node];
    internal double ApplyDirectExchangeInventory(GeodesicOceanResource resource, int node, double inventoryDelta)
    {
        int offset = (int)resource * nodeCapacity + node; double volume = sourceGrid.PhysicalLayerVolumeKm3[node];
        float before = concentrationsByResourceThenNode[offset];
        double desired = Math.Max(0d, before * volume + inventoryDelta);
        float after = (float)Math.Min(float.MaxValue, desired / volume);
        concentrationsByResourceThenNode[offset] = after;
        double actual = (after - (double)before) * volume;
        if (actual != 0d) resourceMayHaveSpatialVariation[(int)resource] = true;
        return actual;
    }
    internal double VentH2Rate => physicalVentH2InventoryRatePerSecond;
    internal double VentH2SRate => physicalVentH2SInventoryRatePerSecond;
    internal double VentCO2Rate => physicalVentCO2InventoryRatePerSecond;
    internal double VentFe2Rate => physicalVentFe2InventoryRatePerSecond;
    ...
    private void ResolveAtmosphereComponents()
    {
        atmosphereField = GetComponent<GeodesicAtmosphereField>();
        if (atmosphereField == null) atmosphereField = gameObject.AddComponent<GeodesicAtmosphereField>();
        airSeaGasExchange = GetComponent<GeodesicAirSeaGasExchange>();
        if (airSeaGasExchange == null) airSeaGasExchange = gameObject.AddComponent<GeodesicAirSeaGasExchange>();
    }
    ...
    {
        withdrawnInventory = 0d; if (!Finite(requestedInventory) || requestedInventory < 0d) { if (!Finite(requestedInventory)) rejectedNonfiniteWriteCount++; else rejectedNegativeWriteCount++; return false; } if (!TryResolveNode(cellIndex, layerIndex, resource, out int offset)) return false; int node = sourceGrid.GetNodeIndex(cellIndex, layerIndex); double available = concentrationsByResourceThenNode[offset] * (double)sourceGrid.PhysicalLayerVolumeKm3[node]; withdrawnInventory = Math.Min(requestedInventory, available); double next = (available - withdrawnInventory) / sourceGrid.PhysicalLayerVolumeKm3[node]; if (!Finite(next) || next > float.MaxValue) { rejectedNonfiniteWriteCount++; return false; } resourceMayHaveSpatialVariation[(int)resource] = true; concentrationsByResourceThenNode[offset] = (float)next; RecomputeDiagnosticsFor(resource); return true;
    }
    public double GetNodeInventory(int cellIndex, int layerIndex, GeodesicOceanResource resource) => TryGetConcentration(cellIndex, layerIndex, resource, out float c) ? c * sourceGrid.PhysicalLayerVolumeKm3[sourceGrid.GetNodeIndex(cellIndex, layerIndex)] : 0d;
    ...
    public bool ValidateField(out string report)
    {
        ...
        int active = 0; double volume = 0d; for (int cell = 0; cell < sourceGrid.CellCount; cell++)
        {
            ...
            {
                int node = sourceGrid.GetNodeIndex(cell, layer); bool isActive = sourceGrid.IsNodeActive(cell, layer); if (isActive) { active++; volume += sourceGrid.PhysicalLayerVolumeKm3[node]; }
        ...
        float[] saved = concentrationsByResourceThenNode; int[] savedActive = activeNodeIndices; double[] savedVolumes = activeNodeVolumes; GeodesicOceanLayerGrid savedGrid = sourceGrid; int savedNodeCapacity = nodeCapacity;
        ...
        RecomputeDiagnostics(); report = errors == 0 ? $"valid; cells={cellCount}; nodeCapacity={nodeCapacity}; activeNodes={activeNodeCount}; physicalVolumeKm3={activeOceanVolume:G6}; memory={approximateRuntimeMemoryBytes} bytes; sentinel=pass; cleanup=pass; reinit=pass" : $"invalid; errors={errors}; first={first}"; return errors == 0;
    }
    ...
    private void ValidateVentContextMenu()
    {
        ...
        double duration = 10d;
        double h2 = physicalVentH2InventoryRatePerSecond * duration, h2s = physicalVentH2SInventoryRatePerSecond * duration, co2 = physicalVentCO2InventoryRatePerSecond * duration, fe2 = physicalVentFe2InventoryRatePerSecond * duration;
